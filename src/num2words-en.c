...
static size_t append_number(char* words, int num, bool o) {
...
void full_date_to_chars(int wday, int month, int day, char *date_line, size_t length) {
  size_t remaining = length;
  memset(date_line, 0, length);
  remaining -= append_string(date_line, remaining, DAYS[wday]);
  remaining -= append_string(date_line, remaining, " ");
  remaining -= append_string(date_line, remaining, MONTHS[month]);
  remaining -= append_string(date_line, remaining, " ");
  remaining -= append_string(date_line, remaining, DATES[day]);
